...
(* $Id: typecore.ml 10860 2010-11-25 21:43:08Z lefessan $ *)
...
(*        | 'r' ->
        ...
        | '{' | '(' as c ->
          let j = j + 1 in
          if j >= len then incomplete_format fmt else
          let sj = assert false (* -fpottier: quick fix
  ...
  | Pexp_function (l, Some default, [spat, sbody]) ->
      ...
      let scases = [
         {ppat_loc = default_loc;
          ppat_desc =
            Ppat_construct
              ((Longident.Ldot (Longident.Lident "*predef*", "Some")),
         ...
         {ppat_loc = default_loc;
          ppat_desc = Ppat_construct
            (Longident.Ldot (Longident.Lident "*predef*", "None"), None, false)},
         default;
      ] in
      let smatch = {
        pexp_loc = loc;
