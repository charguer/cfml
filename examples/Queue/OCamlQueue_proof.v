...
Require Import OCamlQueue_ml.

Definition CellFields A `{Enc A} (x:A) (n:cell_ A) (c:cell_ A) : hprop :=
  \exists cf,
    \[c = Cons cf] \* (cf ~~~> `{ content' := x; next' := n }).

Fixpoint Cell A `{Enc A} (cl:cell_ A) (L:list A) (c:cell_ A) : hprop :=
  match L with
  | nil => \[c = cl]
  | x::L' => \exists n, (c ~> CellFields x n) \* (n ~> Cell cl L')
  end.

Definition Queue A `{Enc A} (L:list A) (p:loc) : hprop :=
  \exists cf cl nb,
      \[nb = LibList.length L] \*
       (p ~~~> `{ length' := nb; first' := cf; last' := cl }) \*
       If L = nil then \[cf = Nil /\ cl = Nil]
       else
         \exists x L', \[L = L' & x] \* (cf ~> Cell cl L') \*
                   (cl ~> CellFields x Nil).
