...
let pwsek_create d oo =
  let pa0 = swchunk_create d oo in
  { p_sides = [| pa0; pa0 |];
    p_mid = None;
    p_weight = 0 }
...
let rec pwsek_push : 'a. view -> owner option -> 'a pwsek -> 'a weighted -> 'a pwsek = fun v o s x ->
  ...
  let front1, mid1 =
    if swchunk_is_full front then begin
      ...
      let m = pwsek_get_mid d o mid in
...
let rec pwsek_concat : 'a. owner option -> 'a pwsek -> 'a pwsek -> 'a pwsek = fun o s1 s2 ->
  ...
  let m = pwsek_get_mid d o s1.p_mid in
...
let rec pwsek_split : 'a. owner option -> 'a pwsek -> int -> 'a pwsek * 'a pwsek = fun o s w ->
  ...
  let front1, mid1, back1, front2, mid2, back2 =
    if w <= wf then
      ...
      let w' = w - wf in
      if w - wf < wm then begin
        match mid with
        | None -> assert false (* Impossible as wm > 1 *)
        | Some m ->
          let m1, m2 = pwsek_split o m w' in
          let m2', f2 = pwsek_pop Front o m in (* non empty as m1.p_weight <= w - wf < wm so m2.p_weight > 0 *)
          let b, f = swchunk_split o f2 (w' - m1.p_weight) in
          front, pwsek_mk_mid m1, b, f, pwsek_mk_mid m2', back
      end else begin
        let b, f = swchunk_split o back (w - wf - wm) in
        front, mid, b, f, None, empty ()
      end
    end
  in
  mk_pwsek_weight_populated Front o front1 mid1 back1, mk_pwsek_weight_populated Front o front2 mid2 back2
