...
(** This file provides a pretty-much end-to-end construction of
    ...
    (the function named [wpgen] in \CHAP{WPgen}), for a core
...
(** Smart constructors for recursive functions with exactly one argument *)

Definition trm_fix_fun (f:var) (x:var) (t1:trm) : trm :=
  trm_fix f (trm_fun x t1).

Definition val_fix_fun (f:var) (x:var) (t1:trm) : val :=
  val_fix f (trm_fun x t1).
...
Fixpoint subst (y:var) (v':val) (t:trm) : trm :=
  ...
  | trm_fix f t1 => trm_fix f (if_y_eq f t1 (aux t1))
...
Inductive eval : heap -> trm -> heap -> val -> Prop :=
  ...
  | eval_fix : forall s f t1,
      eval s (trm_fix f t1) s (val_fix f t1)
  ...
  | eval_app_fix : forall s1 s2 v1 v2 f t1 v,
      v1 = val_fix f t1 ->
      eval s1 (trm_app (subst f v1 t1) v2) s2 v ->
      eval s1 (trm_app v1 v2) s2 v
...
Lemma eval_app_args' : forall s1 s2 s3 s4 t1 t2 v1 v2 r,
  eval s1 t1 s2 v1 ->
  eval s2 t2 s3 v2 ->
  eval s3 (trm_app v1 v2) s4 r ->
  eval s1 (trm_app t1 t2) s4 r.
Proof using.
  ...
  { tests C2: (trm_is_val t2).
    { destruct t1; tryfalse. destruct t2; tryfalse. inverts E1. inverts* E2. } 
    { applys* eval_app_args. } }
 { applys* eval_app_args. }
Qed.

(** Derived rule for the evaluation of recursive functions. *)

Lemma eval_fix_fun : forall s f x t,
  eval s (trm_fix_fun f x t) s (val_fix_fun f x t).
Proof using. intros. applys eval_fix. Qed.

Lemma eval_app_fix_fun : forall s1 s2 v1 v2 f x t1 v,
  v1 = val_fix_fun f x t1 ->
  f <> x ->
  eval s1 (subst x v2 (subst f v1 t1)) s2 v ->
  eval s1 (trm_app v1 v2) s2 v.
Proof using.
  introv E N M. applys eval_app_fix E.
  simpl. rewrite var_eq_spec. case_if.
  applys eval_app_args. { left*. } { applys eval_fun. } { applys eval_val. }
  applys* eval_app_fun.
Qed.
...
(** We next define heap predicates and establish their properties.
    ...
    the functor from \CHAP{LibSepSimpl} that defines the tactic [xsimpl]. *)
Module SepSimplArgs.
...
(** We are now ready to instantiate the functor that defines [xsimpl].
    Demos of [xsimpl] are presented in chapter \CHAP{Himpl.v}. *)
...
(** In this file, we set up an affine logic. The lemma [haffine_any] asserts
    ...
    [xaffine] tactic works in chapter \CHAP{Affine}. *)
...
Lemma hoare_fix_fun : forall f x t1 H Q,
  H ==> Q (val_fix_fun f x t1) ->
  hoare (trm_fix_fun f x t1) H Q.
Proof using.
  introv M. intros h K. exists h __. splits.
  { applys~ eval_fix_fun. }
  { applys* M. }
...
(* INSTRUCTORS: the next 2 lemmas can be bypassed using [wp_eval_like].
...
Lemma hoare_app_fix_fun: forall v1 v2 f x t1 H Q,
  v1 = val_fix_fun f x t1 ->
  f <> x ->
  hoare (subst x v2 (subst f v1 t1)) H Q ->
  hoare (trm_app v1 v2) H Q.
Proof using.
  introv E N M. intros s K0. forwards (s'&v&R1&K1): (rm M) K0.
  exists s' v. splits. { applys eval_app_fix_fun E N R1. } { applys K1. }
Qed.
...
(* INSTRUCTORS: all the structural reasoning rules and the reasoning rules
   for terms are not needed for setting up [wpgen]. *)
...
Lemma triple_fix_fun : forall f x t1 H Q,
  H ==> Q (val_fix_fun f x t1) ->
  triple (trm_fix_fun f x t1) H Q.
Proof using.
  introv M. intros HF. applys~ hoare_fix_fun. { xchanges M. }
Qed.
...
Lemma triple_app_fix_fun : forall v1 v2 f x t1 H Q,
  v1 = val_fix_fun f x t1 ->
  f <> x ->
...
Proof using.
  (* can also be proved using [triple_eval_like] *)
  unfold triple. introv E N M1. intros H'.
  applys hoare_app_fix_fun E N. applys M1.
Qed.

Lemma triple_app_fix_direct : forall v2 f x t1 H Q,
  f <> x ->
  triple (subst x v2 (subst f (val_fix_fun f x t1) t1)) H Q ->
  triple (trm_app (val_fix_fun f x t1) v2) H Q.
Proof using. introv N M. applys* triple_app_fix_fun. Qed.
...
Lemma wp_fix_fun : forall f x t Q,
  Q (val_fix_fun f x t) ==> wp (trm_fix_fun f x t) Q.
Proof using. intros. unfold wp. xsimpl; intros H'. applys hoare_fix_fun. xsimpl. Qed.
...
Lemma wp_app_fix_fun : forall f x v1 v2 t1 Q,
  v1 = val_fix_fun f x t1 ->
  f <> x ->
  wp (subst x v2 (subst f v1 t1)) Q ==> wp (trm_app v1 v2) Q.
Proof using. introv EQ1. unfold wp. xsimpl; intros. applys* hoare_app_fix_fun. Qed.
(* variant: introv EQ1. applys wp_eval_like. introv R. applys* eval_app_fix_fun. *)
...
Definition wpgen_fix_fun (Fof:val->val->formula) : formula := fun Q =>
...
Fixpoint wpgen (E:ctx) (t:trm) : formula :=
  ...
  | trm_fix f t0 => 
      ...
      | trm_fun x t1 => 
          if var_eq f x
             then wpgen_fail
             else wpgen_fix_fun (fun vf v => wpgen ((f,vf)::(x,v)::E) t1)
      | _ => wpgen_fail
      end
...
(* ################################################ *)
(** *** Induction principle for reasoning about [wpgen] *)

(** When [wpgen] is invoked on a recursive function of the form 
    [trm_fix f (trm_fun x t1)], the recursive call is made on [t1],
    which is a subterm of [t] yet not an immediate subterm of [t].
    As a result, the default induction principle associated with terms 
    is insufficient. We next state and prove the appropriate principle. *)
...
(H1: forall (v:val), P v)
(H2: forall (x:var), P x)
(H3: forall x t, P t -> P (trm_fun x t))
(H4a: forall f x t1, f <> x -> P t1 -> P (trm_fix f (trm_fun x t1)))
(H4b: forall f t1, P t1 -> P (trm_fix f (trm_fun f t1)))
(H4c: forall f t0, (forall x t1, t0 <> trm_fun x t1) -> P t0 -> P (trm_fix f t0))
(H5: forall t, P t -> forall t0, P t0 -> P (t t0)) 
(H6: forall t, P t -> forall t0, P t0 -> P (trm_seq t t0)) 
(H7: forall (x : var) t, P t -> forall t0, P t0 -> P (trm_let x t t0)) 
(H8: forall t, P t -> forall t0, P t0 -> forall t1, P t1 -> P (trm_if t t0 t1)).

Program Fixpoint trm_induct t : P t :=
  let F := trm_induct in
  match t as t' return (P t') with
  | trm_val v => H1 v
  | trm_var x => H2 x
  | trm_fun x t0 => H3 x (F t0)
  | trm_fix f t0 => 
      match t0 with
      | trm_fun x t1 => 
          If f = x 
  ...
  | trm_app t0 t1 => H5 (F t0) (F t1)
  | trm_seq t0 t1 => H6 (F t0) (F t1)
  | trm_let x t0 t1 => H7 x (F t0) (F t1)
  | trm_if t0 t1 t2 => H8 (F t0) (F t1) (F t2)
  end.

Lemma trm_ind' : forall t, P t.
Proof. intros. eapply trm_induct. Qed.
...
Lemma wpgen_fix_fun_sound : forall f x t1 Fof,
  (forall vf vx, formula_sound (subst x vx (subst f vf t1)) (Fof vf vx)) ->
  f <> x ->
  formula_sound (trm_fix_fun f x t1) (wpgen_fix_fun Fof).
Proof using.
  introv M N. intros Q. unfolds wpgen_fix_fun.
  applys himpl_hforall_l (val_fix_fun f x t1). xchange hwand_hpure_l.
  { intros. applys himpl_trans_r. { applys* wp_app_fix_fun. } { applys* M. } }
  { applys wp_fix_fun. }
Qed.
...
Proof using.
  intros. gen E.
  eapply trm_ind' with (P := fun t => 
    forall E, formula_sound (isubst E t) (wpgen E t)); intros; simpl;
  ...
  { case_var. applys* wpgen_fix_fun_sound. 
    { fold isubst. intros vf vx. rewrite* <- isubst_rem_2. } }
  { case_var. applys wpgen_fail_sound. }
  { destruct t0; try applys wpgen_fail_sound.
    case_var; try applys wpgen_fail_sound. false*. }
  { applys wp_sound. }
...
Lemma xwp_lemma_fix_fun : forall v1 v2 f x t H Q,
  v1 = val_fix_fun f x t ->
  f <> x ->
...
(** The tactic are presented in \CHAP{WPgen}. *)
...
(** [xapp_apply_spec] implements the heart of [xapp], when called without
    ...
    hypothesis. Disclaimer: as explained in \CHAP{WPgen}, the simple
...
(* SOONER: CUSTOM-ENTRY
   Often in printing there are extra parentheses that show up,
   because the rule below is set only-parsing. But we can't activate
   the rule for printing because it messes up with the coercion trm_app,
   which disables all other printing for primitive operations.
   Maybe it should be at level 1? might be related to a bug reported
   in plf/STLC.v *)
...
(* SOONER: CUSTOM-ENTRY: why would I need a recursive notation?

  Notation "f x .. y" := (.. (trm_app f x) .. y)
    (in custom trm at level 1,
     f custom trm,
     x custom trm at level 9,
     y custom trm at level 9) : trm_scope.
*)
...
Notation "'fix' f x1 x2 .. xn '=>' t" :=
  (val_fix f (trm_fun x1 (trm_fun x2 .. (trm_fun xn t) ..)))
  (in custom trm at level 69,
   t custom trm,
   f, x1, x2, xn at level 0,
...
(* LATER: CUSTOM-ENTRY: how to remove underscore in fun and fix?
   could perhaps achieve that by introducing 'custom val'.
   For now, the use of 'let f x = ...' and 'let rec f x = ...'
...
Notation "'fix_' f x1 x2 .. xn '=>' t" :=
  (trm_fix f (trm_fun x1 (trm_fun x2 .. (trm_fun xn t) ..)))
  (in custom trm at level 69,
   t custom trm,
   f, x1, x2, xn at level 0,
   format "'fix_'  f  x1  x2  ..  xn  '=>'  t") : trm_scope.

(* LATER: should we add left associativity for functions? *)
...
Fixpoint trm_funs (L:vars) (t:trm) : trm :=
  match L with 
  | nil => t
  | x1::L' => trm_fun x1 (trm_funs L' t)
  end.

(** [val_funs xs t] is similar to [trm_funs xs t] but produces a value. *)

Definition val_funs (L:vars) (t:trm) : val :=
  match L with 
  | nil => arbitrary
  | x1::L' => val_fun x1 (trm_funs L' t)
  end.
...
Proof using.
  introv R Nx Hx M. applys eval_app_args'.
  { applys R. applys eval_val. }
  { applys eval_val. }
  { applys* eval_app_fun. rewrite* subst_trm_funs. applys* eval_like_trm_funs. }
Qed.
...
Proof using.
  introv R N. applys eval_app_args'.
  { applys R. applys eval_val. }
  { applys eval_val. }
  { applys* eval_app_fun. }
Qed.
...
Lemma eval_like_trm_apps_fix_funs : forall f xs vs t1 v0,
  v0 = val_fix f (trm_funs xs t1) ->
  var_funs xs (length vs) ->
  ~ mem f xs ->
  xs <> nil ->
...
Proof using.
  ...
  { destruct vs' as [|]; tryfalse.
    ...
    introv M. applys* eval_app_fix. rewrite* subst_trm_funs. simpl.
    applys eval_app_args'. { applys eval_fun. } { applys eval_val. }
    { applys* eval_app_fun. } }
  { rew_listx. rewrite isubst_cons. simpl. rew_listx in N1.
    ...
    { introv M. applys* eval_app_fix. simpl. case_var.
      inverts M. rewrite* subst_trm_funs.
      applys eval_app_args'. applys eval_fun. applys eval_val.
      applys eval_app_fun. reflexivity. rewrite* subst_trm_funs.
...
(** [val_fix_funs_of_trm t] is a tactic that takes a term [t] and returns a
    corresponding term in the form [val_fix (val_funs xs t1)], if possible. *)

Ltac val_fix_funs_of_trm t :=
  let t := eval hnf in t in
  match t with  
  | val_fix ?f ?t1 =>
      match trm_funs_of_trm t1 with
      | trm_funs ?xs ?t0 => constr:(val_fix f (trm_funs xs t0))
      end
  | _ => constr:(t)
  end.
...
Ltac prove_eq_val_fix_trm_funs :=
  match goal with |- ?t = val_fix _ (trm_funs _ _) =>
    let r := val_fix_funs_of_trm t in
    apply (refl_equal r) end.

Lemma prove_eq_val_fix_trm_funs_demo : forall (x1 x2:var) (t0:trm), 
  (forall f xs t, val_fix x1 (trm_fun x2 t0) = val_fix f (trm_funs xs t) -> (f,xs,t) = (f,xs,t) -> True) ->
  True.
Proof using. intros. eapply H. prove_eq_val_fix_trm_funs. Abort.


(* ########################################################### *)
(** ** Definition of the tactic [xwp] *)
...
Proof using.
  ...
  rewrite <- wp_equiv. xchange (rm M2). xchange wpgen_sound.
  applys wp_eval_like. applys* eval_like_trm_apps_fix_funs.
Qed.
...
(** The chapter \CHAP{Struct} shows how to these specifications may be
    realized. *)
...
(* SOONER: CUSTOM-ENTRY
   above, if I execute [unfold incr], why don't the single quotes show up? *)
...
(* SOONER: CUSTOM-ENTRY
   why does [Print mysucc] prints [(incr:trm)] whereas this
   is not visible after an unfold in a goal? *)
...
(* SOONER: CUSTOM-ENTRY: For debugging:  unfold mysucc; xvars. *)
...
(* ########################################################### *)
(** *** Demo of a Functions with Multiple Arguments. *)
