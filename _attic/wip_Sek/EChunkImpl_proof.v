...
Lemma echunk_fields_spec : forall (A: Type) (IA: Inhab A) (EA: Enc A) (L: list A) (c: echunk_ A),
	SPEC (echunk_fields c)
		...
		POST (fun '(data, front, size, default) => \exists D,
			...
			data ~> Array D \*
			\[EChunk_inv L D front size default]).
Proof. xcf*. xopen c ;=> data D front size default I. xappn*. xvals*. Qed.
...
Proof. introv I. xcf. xapp ;=> c. xchanges~ <- EChunk_eq. Qed.
...
Lemma echunk_default_spec : forall (A: Type) (IA: Inhab A) (EA: Enc A) (L: list A) (c: echunk_ A),
	SPEC (echunk_default c)
		...
		INV (c ~> EChunk L)
		POST (fun (d: A) => \[]).
Proof. xcf. xopen c ;=> a D f s d I. xapp. xclose* c. xsimpl. Qed.
...
Ltac xcf_post tt := idtac.
  xcf. xlet. xapp_types. xapp_spec. xlet. xapp. xapp.
  xcf_nopay. xapp~ ;=> a L ->.
  xapp~; try xsimpl~.
...
Lemma echunk_size_spec : forall (A: Type) (IA: Inhab A) (EA: Enc A) (L: list A) (c: echunk_ A),
	SPEC (echunk_size c)
		...
		INV (c ~> EChunk L)
		POST \[= length L].
Proof. xcf~. xopen c ;=> a D f s d I. xapp. xclose* c. xsimpl. forwards*: ecinv_length. Qed.
...
Proof.
	introv HL.
	xcf*. lets (x & q & ->): list_neq_nil_inv_cons L HL.
